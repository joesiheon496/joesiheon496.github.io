...
> 🎯 **커버 규칙 (논문 글)**: 커버(`[cover].image`)는 **논문의 개념/overview figure**(있으면)를 씁니다.
> 세부 **pipeline/architecture 그림은 본문**으로 옮깁니다. arXiv HTML판(`arxiv.org/html/<id>`)에서 그림을 받아
> 글 폴더에 넣고, **캡션에 출처(저자·arXiv 링크)를 명기**하세요. 개념 보충용 자작 도식은 본문에 별도로.
>
> 🎯 **부록 규칙 (논문 글)**: 글 맨 뒤에 `## 부록: 수식 직관 가이드` 를 두고, 핵심 수식마다
> **비유 + 간단한 자작 그림**으로 직관을 덧붙입니다(선택 코너). 본문 흐름은 가볍게 유지하고 깊은 설명은 여기로.

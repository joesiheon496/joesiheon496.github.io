...
- **블록 수식 안에 `=` 를 홀로 한 줄에 두지 않는다.** Markdown 이 setext 제목 밑줄로 해석해
  ...
  `=` 는 앞 줄 끝에 붙인다.
- **본문 물결표는 `\~` 로 이스케이프한다.** goldmark 의 strikethrough 는 `~~` 뿐 아니라
  **한 개짜리 `~` 도 구분자로 받는다.** 그래서 한 문단에 물결표가 둘 있으면 그 사이가 통째로
  취소선이 된다. 4편에서 `수백~수천` 과 `0~2π` 가 짝을 이뤄 `<del>수천)와 회전(라디안 단위, 0</del>`
  이 되었고, 빌드 오류도 경고도 없이 지나가 사람이 브라우저에서 발견했다.
  범위 표기가 필요하면 `\~` 로 쓴다 — 화면에는 똑같이 `~` 로 보인다.
  검증: 빌드 산출물에 `<del>` 이 0 건이어야 한다.
...
test('시작점 [1,1] 은 θ=45° 에서 고유벡터라 한 스텝에 끝난다 (인공물 회귀 테스트)', () => {
  ...
  // ⚠️ 위 단정들은 시작점이 하나뿐이라 "평균에 대해 η 를 고른다" 와 "시작점마다 고른다" 를
